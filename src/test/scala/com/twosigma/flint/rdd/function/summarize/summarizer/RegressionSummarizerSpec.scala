...
class RegressionSummarizerSpec extends FlatSpec {
  ...
  "RegressionSummarizer" should "transform from RegressRow correctly" in {
    ...
    val (response1, predictor1, yw1) = RegressionSummarizer.transform(
      x,
      shouldIntercept = true,
      isWeighted = true
    )
    assert(
      response1.equals(DenseMatrix(Array(2d, 2d, 4d), Array(4d, 16d, 20d)))
    )
    assert(predictor1.equals(DenseVector(Array(6d, 24d))))
    assert(yw1.deep == Array((3d, 4d), (6d, 16d)).deep)

    val (response2, predictor2, yw2) = RegressionSummarizer.transform(
      x,
      shouldIntercept = true,
      isWeighted = false
    )
    ...
    val (response3, predictor3, yw3) = RegressionSummarizer.transform(
      x,
    ...
    val (response4, predictor4, yw4) = RegressionSummarizer.transform(
      x,
      shouldIntercept = false,
      isWeighted = false
    )
