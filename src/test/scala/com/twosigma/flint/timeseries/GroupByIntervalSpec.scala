...
class GroupByIntervalSpec extends MultiPartitionSuite with TimeSeriesTestData {
  ...
  "GroupByInterval" should "group by clock correctly" in {
    ...
    val expectedResults = TimeSeriesRDD.fromDF(
      spark.createDataFrame(
        sc.parallelize(
          Array[Row](
            new GenericRowWithSchema(
              Array(1100L, Array(rows(0), rows(1), rows(2), rows(3))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1200L, Array(rows(4), rows(5), rows(6), rows(7))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1300L, Array(rows(8), rows(9), rows(10), rows(11))),
              expectedSchema
            )
          )
        ),
    ...
    )(isSorted = true, timeUnit = TimeUnit.NANOSECONDS)
    ...
    val expectedResults2 = TimeSeriesRDD.fromDF(
      spark.createDataFrame(
        sc.parallelize(
          Array[Row](
            new GenericRowWithSchema(
              Array(1050L, Array(rows(0), rows(1))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1100L, Array(rows(2), rows(3))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1150L, Array(rows(4), rows(5))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1200L, Array(rows(6), rows(7))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1250L, Array(rows(8), rows(9))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1300L, Array(rows(10), rows(11))),
              expectedSchema
            )
          )
        ),
  ...
  it should "group by clock correctly with with (inclusion, rounding) = (end, end)" in {
    ...
    val expectedResults = TimeSeriesRDD.fromDF(
      spark.createDataFrame(
        sc.parallelize(
          Array[Row](
            new GenericRowWithSchema(
              Array(1000L, Array(rows(0), rows(1))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1100L, Array(rows(2), rows(3), rows(4), rows(5))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1200L, Array(rows(6), rows(7), rows(8), rows(9))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1300L, Array(rows(10), rows(11))),
      ...
      spark.createDataFrame(
        sc.parallelize(
          Array[Row](
            ...
            new GenericRowWithSchema(
              Array(1050L, Array(rows(2), rows(3))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1100L, Array(rows(4), rows(5))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1150L, Array(rows(6), rows(7))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1200L, Array(rows(8), rows(9))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1250L, Array(rows(10), rows(11))),
              expectedSchema
            )
          )
        ),
    ...
    def test(rdd: TimeSeriesRDD): Unit = {
      val results =
        rdd.groupByInterval(clockTSRdd, inclusion = "end", rounding = "end")
      assertEquals(results, expectedResults)

      val results2 =
        rdd.groupByInterval(clockTSRdd2, inclusion = "end", rounding = "end")
      assertEquals(results2, expectedResults2)
    }
    ...
    val expectedResults = TimeSeriesRDD.fromDF(
      spark.createDataFrame(
        sc.parallelize(
          Array[Row](
            new GenericRowWithSchema(
              Array(1100L, 7, Array(rows(0), rows(3))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1100L, 3, Array(rows(1), rows(2))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1200L, 3, Array(rows(4), rows(6))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1200L, 7, Array(rows(5), rows(7))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1300L, 3, Array(rows(8), rows(10))),
              expectedSchema
            ),
            new GenericRowWithSchema(
              Array(1300L, 7, Array(rows(9), rows(11))),
              expectedSchema
            )
          )
        ),
  ...
  it should "pass cycle data property test" in {
    ...
    for (
      ...
      withPartitionStrategyCompare(testData)(DEFAULT)(
        sum(width, key, inclusion, rounding)
      )
    }
  }
}
