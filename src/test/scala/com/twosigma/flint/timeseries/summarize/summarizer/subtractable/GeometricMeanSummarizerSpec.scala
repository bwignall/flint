...
class GeometricMeanSummarizerSpec extends SummarizerSuite {
  ...
  override val defaultResourceDir: String =
    "/timeseries/summarize/summarizer/geometricmeansummarizer"
  ...
  "GeometricMeanSummarizer" should "compute geometric mean correctly" in {
    ...
    val results = priceTSRdd
      .summarize(Summarizers.geometricMean("price"), Seq("id"))
      .collect()
    assert(
      results
        .find(_.getAs[Int]("id") == 3)
        .head
        .getAs[Double]("price_geometricMean") === 2.621877636494
    )
    assert(
      results
        .find(_.getAs[Int]("id") == 7)
        .head
        .getAs[Double]("price_geometricMean") === 2.667168275340
    )
  }
  ...
  it should "compute geometric mean with a zero correctly" in {
    ...
    var results =
      priceTSRdd.summarize(Summarizers.geometricMean("priceWithZero")).collect()
    ...
    assert(
      results.last
        .getAs[Double]("priceWithZero_geometricMean") === 5.220043408524
    )
  }
  ...
  it should "compute geometric mean with negative values correctly" in {
    ...
    val results = priceTSRdd
      .summarize(Summarizers.geometricMean("priceWithNegatives"), Seq("id"))
      .collect()
    assert(
      results
        .find(_.getAs[Int]("id") == 3)
        .head
        .getAs[Double]("priceWithNegatives_geometricMean")
        === -2.621877636494
    )
    assert(
      results
