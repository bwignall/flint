...
class CorrelationSummarizerSpec extends SummarizerSuite {
  ...
  private lazy val init: Unit = {
    ...
    input = priceTSRdd
      .leftJoin(forecastTSRdd, key = Seq("id"))
      .addColumns(
  ...
  "CorrelationSummarizer" should "compute correlation correctly" in {
    ...
    assert(
      results
        .find(_.getAs[Int]("id") == 7)
        .head
        .getAs[Double]("price_forecast_correlationTStat") ===
        -0.04380274440368827
    )
    assert(
      results
    ...
    results = input
      .summarize(
        Summarizers.correlation("price", "price3", "forecast"),
        Seq("id")
      )
      .collect()
    assert(
      results
        .find(_.getAs[Int]("id") == 7)
        .head
        .getAs[Double]("price_price3_correlation") ===
        -1.0
    )
    assert(
      results
        .find(_.getAs[Int]("id") == 7)
        .head
        .getAs[Double]("price_price3_correlationTStat") ==
    ...
    assert(
      results
        .find(_.getAs[Int]("id") == 7)
        .head
        .getAs[Double]("price3_forecast_correlation") ===
        0.021896121374023046
    )
    assert(
      results
    ...
    assert(
      results
        .find(_.getAs[Int]("id") == 7)
        .head
        .getAs[Double]("price3_forecast_correlationTStat") ===
        0.04380274440368827
    )

    results = input
      .summarize(
        Summarizers.correlation(Seq("price", "price3"), Seq("forecast")),
        Seq("id")
      )
      .collect()
    assert(
    ...
    intercept[RuntimeException] {
      input.summarize(
        Summarizers
          .correlation(Seq("price", "price3"), Seq("forecast", "price3"))
      )
    }
  }
  ...
  it should "pass summarizer property test" in {
    summarizerPropertyTest(AllPropertiesAndSubtractable)(
      Summarizers.correlation("x1", "x2")
    )
    summarizerPropertyTest(AllPropertiesAndSubtractable)(
      Summarizers.correlation("x0", "x3")
    )
  }
}
