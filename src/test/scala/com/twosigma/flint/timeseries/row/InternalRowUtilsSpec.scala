...
class InternalRowUtilsSpec extends FlatSpec with TableDrivenPropertyChecks {
  ...
  val rows = Table(
    ...
    (
      ...
      Schema.of(
        "time" -> LongType,
        "nestedColumn" -> Schema
          .of("string" -> StringType, "double" -> DoubleType)
      )
    )
  )
  ...
  val rowProjections = Table(
    ...
    (row: Row, schema: StructType) =>
      CatalystTypeConvertersWrapper.toCatalystRowConverter(schema)(row),
    (row: Row, schema: StructType) => {
      val internalRow =
        CatalystTypeConvertersWrapper.toCatalystRowConverter(schema)(row)
  ...
  "InternalRowUtils" should "select indices correctly" in {
    testTemplate { (data: Row, schema: StructType, row: InternalRow) =>
      ...
      val converted = CatalystTypeConvertersWrapper.toScalaRowConverter(schema)(
        InternalRow.fromSeq(selected)
      )
  ...
  it should "delete correctly" in {
    testTemplate { (data: Row, schema: StructType, row: InternalRow) =>
      val (deleteFn, newSchema) =
        InternalRowUtils.delete(schema, Seq(schema.fieldNames(0)))

      assert(newSchema.fields sameElements schema.fields.tail)

      val updatedRow = deleteFn(row)
      val converted =
        CatalystTypeConvertersWrapper.toScalaRowConverter(newSchema)(updatedRow)
  ...
  it should "select correctly" in {
    testTemplate { (data: Row, schema: StructType, row: InternalRow) =>
      val (selectFn, newSchema) =
        InternalRowUtils.select(schema, Seq(schema.fieldNames(0)))
  ...
  it should "add correctly" in {
    testTemplate { (data: Row, schema: StructType, row: InternalRow) =>
      val toAdd: Seq[(String, DataType)] =
        Seq("long" -> LongType, "double" -> DoubleType, "string" -> StringType)
      val values = Seq(5L, 3.0, UTF8String.fromString("test"))
      val (addFn, newSchema) = InternalRowUtils.add(schema, toAdd)

      val expectedFileds = schema.fields ++ toAdd.map {
  ...
  it should "addOrUpdate correctly" in {
    testTemplate { (data: Row, schema: StructType, row: InternalRow) =>
      val toAdd1: Seq[(String, DataType)] =
        Seq("time" -> LongType, "newColumnName" -> DoubleType)
      val values1 = IndexedSeq[Any](5L, 3.0)
      val (addOrUpdateFn1, newSchema1) =
        InternalRowUtils.addOrUpdate(schema, toAdd1)

      // we are expecting that time column exists in the rows
      val expectedFields1 = schema.fields ++ toAdd1.tail.map {
        case (name, dataType) => StructField(name, dataType)
      }
      assert(newSchema1.fields sameElements expectedFields1)

      val updatedRow1 = addOrUpdateFn1(row, values1)
      val converted1 = CatalystTypeConvertersWrapper.toScalaRowConverter(
        newSchema1
      )(updatedRow1)
      ...
      val (addOrUpdateFn2, newSchema2) =
        InternalRowUtils.addOrUpdate(newSchema1, toAdd2)
      val updatedRow2 = addOrUpdateFn2(updatedRow1, values2)
      val expectedFields2 = schema.fields ++ toAdd2.tail.map {
        case (name, dataType) => StructField(name, dataType)
      }
      assert(newSchema2.fields sameElements expectedFields2)
      val converted2 = CatalystTypeConvertersWrapper.toScalaRowConverter(
        newSchema2
      )(updatedRow2)
  ...
  it should "concat2 array correctly" in {
    testTemplate { (data: Row, schema: StructType, row: InternalRow) =>
      val schemaToAdd = Schema.of(
        "uniqueColumnName1" -> LongType,
        "uniqueColumnName2" -> DoubleType
      )
      ...
      val converted = CatalystTypeConvertersWrapper.toScalaRowConverter(
        concatenatedSchema
      )(updatedRow)
  ...
  it should "concat2 correctly" in {
    testTemplate { (data: Row, schema: StructType, row: InternalRow) =>
      ...
      val rowToAdd = InternalRow.fromSeq(Seq[Any](5L, 3.0))
      val (concatFn, concatenatedSchema) =
        InternalRowUtils.concat2(schema, schemaToAdd)
  ...
  it should "update correctly" in {
    testTemplate { (data: Row, schema: StructType, row: InternalRow) =>
      ...
      val converted =
        CatalystTypeConvertersWrapper.toScalaRowConverter(schema)(updatedRow)
