...
class DFConversionSpec extends TimeSeriesSuite with FlintTestData {
  ...
  val clockData = (0L to 100L).map(ts =>
    (ts, new GenericRowWithSchema(Array(ts), clockSchema).asInstanceOf[Row]))
  var clockTSRdd: TimeSeriesRDD = _

  private lazy val init: Unit = {
    clockTSRdd = TimeSeriesRDD.fromOrderedRDD(
      OrderedRDD.fromRDD(
        sc.parallelize(clockData, defaultPartitionParallelism),
  ...
  def genRandomDataFrame(
    ...
    idDf.select(
      timeColumn.as(TimeSeriesRDD.timeColumnName).cast(LongType),
      functions.randn(SEED).as("value")
    )
  }
  ...
  it should "convert time unit correctly" in {
    ...
    assert(zipped.forall {
      case (row, convertedRow) =>
        row.getAs[Long]("time") * 1000 == convertedRow.getAs[Long]("time")
    })
  }
  ...
  it should "convert timestamp correctly" in {
    init
    val df =
      clockTSRdd.toDF.withColumn("time", col("time") * (123456 * 10e8).toLong)
    val convertedDf =
      df.withColumn("time", (df("time") / 10e8).cast(TimestampType))

    // These should all be the same
    val result1 = TimeSeriesRDD.fromDF(convertedDf)(isSorted = true, null)
    val result2 =
      TimeSeriesRDD.fromDF(convertedDf)(isSorted = true, TimeUnit.NANOSECONDS)
    val result3 =
      TimeSeriesRDD.fromDF(convertedDf)(isSorted = true, TimeUnit.MICROSECONDS)
  ...
  it should "sort dataframes correctly" in {
    ...
    val tsRdd =
      TimeSeriesRDD.fromDF(revertedDf)(isSorted = false, TimeUnit.NANOSECONDS)
  ...
  it should "convert to physical rdd" in {
    ...
    var tsRdd =
      TimeSeriesRDD.fromDF(unsortedDF)(isSorted = false, TimeUnit.NANOSECONDS)
    assert(
      PartitionPreservingOperation.isPartitionPreservingDataFrame(tsRdd.toDF)
    )
    tsRdd =
      TimeSeriesRDD.fromDF(sortedDF)(isSorted = true, TimeUnit.NANOSECONDS)
    assert(
      PartitionPreservingOperation.isPartitionPreservingDataFrame(tsRdd.toDF)
    )
  }
  ...
  it should "use timeColumn correctly" in {
    ...
    val tsRdd1 =
      TimeSeriesRDD.fromDF(df1)(isSorted = true, TimeUnit.NANOSECONDS)
    val tsRdd2 = TimeSeriesRDD
      .fromDF(df2)(isSorted = true, TimeUnit.NANOSECONDS, timeColumn = "time2")
  ...
  it should "throw exception if `time` column exists but is not time column" in {
    ...
    intercept[IllegalArgumentException] {
      ...
      TimeSeriesRDD
        .fromDF(df)(isSorted = true, TimeUnit.NANOSECONDS, timeColumn = "time2")
    }
  }
  ...
  it should "throw exception if timeColumn doesn't exist" in {
    init
    intercept[Exception] {
      TimeSeriesRDD.fromDF(testData.withColumnRenamed("time", "time2"))(
        isSorted = true,
  ...
  it should "build from unsorted DataFrames correctly" in {
    ...
    maxValues.foreach { maxValue =>
      ...
      intercept[Exception] {
        TimeSeriesRDD
          .fromDF(df)(isSorted = true, TimeUnit.NANOSECONDS)
          .keepColumns("time")
      }
      TimeSeriesRDD
        .fromDF(df)(isSorted = false, TimeUnit.NANOSECONDS)
        .keepColumns("time")
        .validate()
    }
  }
  ...
  it should "correctly use Catalyst partitioning information" in {
    ...
    assert(
      TimeSeriesStore.isClustered(df.sort("time").queryExecution.executedPlan)
    )

    assert(
      TimeSeriesStore
        .isClustered(df.sort(col("time").desc).queryExecution.executedPlan)
    )
    assert(
      !TimeSeriesStore
        .isSorted(df.sort(col("time").desc).queryExecution.executedPlan)
    )

    assert(
      TimeSeriesStore
        .isClustered(df.repartition(col("time")).queryExecution.executedPlan)
    )
    assert(
      !TimeSeriesStore
        .isSorted(df.repartition(col("time")).queryExecution.executedPlan)
    )

    assert(
      !TimeSeriesStore.isClustered(df.sort("data").queryExecution.executedPlan)
    )

    assert(
      !TimeSeriesStore
        .isSorted(df.sort("data", "time").queryExecution.executedPlan)
    )
  }
  ...
  it should "correctly canonize normalized DataFrame" in {
    ...
    val canonized = TimeSeriesRDD.canonizeDF(
      withWrongOrder,
      isSorted = true,
      TimeUnit.NANOSECONDS,
      "time"
  ...
  it should "preserve partitions of a sorted DF" in {
    ...
    val dfPartitions = sortedDf.rdd
      .mapPartitions { iter =>
        if (iter.isEmpty) Iterator.empty else Iterator(iter.next())
      }
      .collect()
      .map(_.getLong(0))
  ...
  it should "correctly convert a DataFrame sorted by time,id" in {
    val df = clockTSRdd.toDF
      .withColumn("ids", array(lit(2), lit(1), lit(0)))
