...
trait FlintTestData {
  ...
  protected val withTime2Column = { df: DataFrame =>
    df.withColumn("time2", df("time") * 2)
  }
  ...
  protected val selectExprVPlusOne = { df: DataFrame =>
    df.selectExpr("v + 1 as v")
  }
  ...
  protected val addRankColumn = { df: DataFrame =>
    df.withColumn(
      "rank",
      percent_rank().over(Window.partitionBy("time").orderBy("v"))
    )
  }
  protected val selectSumV = { df: DataFrame => df.select(sum("v")) }
  protected val selectExprSumV = { df: DataFrame => df.selectExpr("sum(v)") }
  protected val groupByTimeSumV = { df: DataFrame =>
    df.groupBy("time").agg(sum("v").alias("v"))
  }
