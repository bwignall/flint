...
object ArrowConverters {
  ...
  def toUnsafeRowsIter(
    ...
    val arrowSchema = root.getSchema
    // ArrowPayLoad contains only one batch.
    assert(reader.getRecordBlocks.size() == 1)
    ArrowUtils.assertDataTypeEquals(sparkSchema, arrowSchema)
    reader.loadNextBatch()
    new ArrowBackendUnsafeRowIterator(root, arrowSchema, root.getRowCount)
  }
