...
case class StackSummarizer[T, U, V: ClassTag](
  ...
  override def add(u: Seq[U], t: T): Seq[U] =
    u.zip(summarizers).map { case (u, s) => s.add(u, t) }

  override def merge(u1: Seq[U], u2: Seq[U]): Seq[U] =
    u1.zip(u2)
      .zip(summarizers)
      .map { case ((u1, u2), s) => s.merge(u1, u2) }

  override def render(u: Seq[U]): Seq[V] =
    u.zip(summarizers).map { case (u, s) => s.render(u) }
}
