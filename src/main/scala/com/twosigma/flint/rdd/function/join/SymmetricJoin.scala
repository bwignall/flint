...
protected[flint] object SymmetricJoin {
  ...
  def apply[K: ClassTag, SK, V, V2](
    ...
    new OrderedRDD[K, (Option[(K, V)], Option[(K, V2)])](
      leftRdd.sc,
      mergedSplits,
      ...
      val leftIter = PeekableIterator(
        PartitionsIterator(leftRdd, leftParts, context).filter {
          case (k, _) => mergedJoin.range.contains(k)
        }
      )
      val leftIterFull = PeekableIterator(
        PartitionsIterator(leftRdd, leftParts, context)
      )

      val rightParts = mergedJoin.right.map(_.partition)
      val rightIter = PeekableIterator(
        PartitionsIterator(rightRdd, rightParts, context).filter {
          case (k, _) => mergedJoin.range.contains(k)
        }
      )
      val rightIterFull = PeekableIterator(
        PartitionsIterator(rightRdd, rightParts, context)
      )
